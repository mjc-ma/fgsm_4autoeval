...
from typing import IO, Any, Callable, Dict, List, Optional, Tuple, Union  # <fix>
from torchvision.datasets import MNIST, FashionMNIST, KMNIST  # <fix>
import random  # <fix>


# <fix>, <<Are labels always...>> load from the given numpy file and process into RGB format
...
# <fix>, <<Are labels always...>> load from the given numpy file
...
# <fix>, Override the path and 'L' model of torchvision MNIST
class MyMNISTRAW(MNIST):
    ...
    @property  # <<fix>>, fix the MNIST path: xxx/MNIST/raw
    def raw_folder(self) -> str:
        return os.path.join(self.root, 'MNIST', 'raw')

    @property  # <<fix>>, fix the MNIST path: xxx/MNIST/processed
    def processed_folder(self) -> str:
        return os.path.join(self.root, 'MNIST', 'processed')

    def __getitem__(self, index: int) -> Tuple[Any, Any]:
        img, target = self.data[index], int(self.targets[index])
        img = Image.fromarray(img.numpy()).convert('RGB')  # <<fix>>, original code uses mode='L'
...
# <fix>, Override the path and 'L' model of torchvision FashionMNIST
class MyFashionMNIST(FashionMNIST):
    ...
    @property  # <<fix>>, fix the FashionMNIST path: xxx/FashionMNIST/raw
    def raw_folder(self) -> str:
        return os.path.join(self.root, 'FashionMNIST', 'raw')

    @property  # <<fix>>, fix the FashionMNIST path: xxx/FashionMNIST/processed
    def processed_folder(self) -> str:
...
# <fix>, Override the path and 'L' model of torchvision KMNIST
class MyKMNIST(KMNIST):
    ...
    @property  # <<fix>>, fix the KMNIST path: xxx/KMNIST/raw
    def raw_folder(self) -> str:
        return os.path.join(self.root, 'KMNIST', 'raw')

    @property  # <<fix>>, fix the KMNIST path: xxx/KMNIST/processed
    def processed_folder(self) -> str:
...
# <fix>, copy from the rotation code
...
class MyTinyImageNet(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        # doing this so that it is consistent with all other datasets
        # to return a PIL Image
        img = Image.fromarray(img) # fromarray接受的是【0,255】这个区间的uint8类型的数据
...
def save_checkpoint(state, is_best, filename='checkpoint.pth'):    # <fix>, forbidden to save as tar.gz
    torch.save(state, filename)
    if is_best:
        shutil.copyfile(filename, os.path.split(filename)[0] + '/checkpoint_best.pth')    # <fix>
